...
pub fn assign_cluster(comptime T: type, comptime data: []const T, means: []const T, clusters: []std.ArrayList(T)) ![]std.ArrayList(T) {
    ...
    for (data) |d| {
        ...
        var group = closest_point(means, d);
...
pub fn main() !void {
    var means = get_means(Point, 3, dataset[0..]);
    var clusters = try init_clusters(Point, 3, gpa_allocator);
    var group = try assign_cluster(Point, dataset[0..], means[0..], clusters);
    for (group, 0..) |c, i| {
        print("cluster {d}: {any}\n \n", .{ i + 1, c.items });
    }
    var new_means = try new_centroids(Point, 3, group);
    print("new means: {d:.2}\n \n", .{new_means});
    ...
    clusters = try init_clusters(Point, 3, gpa_allocator);
    var new_group = try assign_cluster(Point, dataset[0..], new_means[0..], clusters);
    for (new_group, 0..) |c, i| {
